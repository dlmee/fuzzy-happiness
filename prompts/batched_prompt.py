...
class BATP:
  ...
  "मुस्लो": {
    ...
    def make_message(self, lang1, lang2, data):
        preamble = f"You are a skilled {lang1} linguist. A {lang2} linguist has come to you with what he believes is a bunch of inflected forms. Your job is to provide the stem (or stems if there are in fact multiple stems), proper noun assessment (either 'yes' or 'no'), stopword assessment (also 'yes' or 'no', and a definition in {lang2} for each stem. For example: If the languages were Nepali and English, and the data was\n{self.example}\nYou would return\n{self.shot}\n"
